    ...
    <AppLayout>
      <div className="space-y-8 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 rounded-2xl border border-slate-900 bg-slate-900/15 p-6 backdrop-blur-md text-center flex flex-col items-center justify-between">
            <div className="w-full flex flex-col items-center">
              ...
              <div className="relative group cursor-pointer mb-5" onClick={() => setAvatarPickerOpen(true)}>
                <img
                  src={activeAvatar}
                  alt="User Avatar"
                  className="h-28 w-28 rounded-full border-2 border-indigo-500 bg-slate-900 object-cover shadow-[0_0_20px_rgba(99,102,241,0.25)] transition-all group-hover:opacity-85"
                  onError={(e) => {
                    (e.target as HTMLImageElement).src = avatarPresets[0].path;
                  }}
                />
                <div className="absolute inset-0 flex items-center justify-center rounded-full bg-slate-950/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <Camera className="h-5 w-5 text-indigo-400" />
                </div>
              </div>

              <h2 className="text-lg font-bold text-slate-200">{name || "Khushi Kumari"}</h2>
              <p className="text-xs text-slate-500 font-semibold mt-0.5">{email || "khushi@example.com"}</p>
              
              <div className="flex items-center gap-1.5 justify-center mt-3 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
            ...
            {/* Level card overlay */}
            <div className="w-full mt-6 pt-5 border-t border-slate-900/80 text-left space-y-3">
              <div className="flex items-center justify-between text-xs font-bold text-slate-350">
                ...
                <span className="text-indigo-400">{xpInCurrentLevel} / 100 XP</span>
              </div>
          ...
          <div className="md:col-span-2 rounded-2xl border border-slate-900 bg-slate-900/15 p-6 backdrop-blur-md">
            <h2 className="text-xs font-extrabold text-slate-400 tracking-wider uppercase mb-5 flex items-center gap-1.5">
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-4">
              <div>
                <label className="mb-1.5 block text-xs font-extrabold uppercase tracking-wide text-slate-400">Full Name</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-800 bg-slate-900/60 pl-11 pr-4 py-3 text-sm text-slate-100 focus:border-indigo-500 focus:outline-none"
                  />
                </div>
              </div>

              <div>
                <label className="mb-1.5 block text-xs font-extrabold uppercase tracking-wide text-slate-400">Email Address</label>
                <div className="relative">
                  <span className="absolute inset-y-0 left-0 pl-4 flex items-center text-slate-600">
        ...
        <div className="rounded-2xl border border-slate-900 bg-slate-900/15 p-6 backdrop-blur-md">
          <h2 className="text-xs font-extrabold text-slate-400 tracking-wider uppercase mb-5 flex items-center gap-1.5">
      ...
      {avatarPickerOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-slate-800 bg-slate-950 p-6 shadow-2xl z-10">
            <h3 className="text-lg font-bold text-slate-200 mb-2">Choose Avatar Profile</h3>
            <p className="text-xs text-slate-500 font-semibold mb-6">Select a preset to customize your visual identity</p>
            ...
            <div className="grid grid-cols-3 gap-4">
              {avatarPresets.map((preset) => (
                ...
                >
                  <img src={preset.path} alt={preset.name} className="h-12 w-12 rounded-full border border-slate-800 bg-slate-900" />
                  <span className="text-[10px] text-slate-400 font-semibold mt-2 text-center leading-tight">{preset.name}</span>
                </button>
              ))}
            </div>
          </div>
